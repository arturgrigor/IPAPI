...
class IPAPI_Tests: XCTestCase {
    ...
    func testCurrentIPAddress() {
        ...
        var result: Swift.Result<Service.Result, Swift.Error>?
        let responseArrived = self.expectation(description: "Response of async request has arrived")
        
        Service.default.fetch {
            result = $0
            responseArrived.fulfill()
        }
        
        self.waitForExpectations(timeout: 15) { inError in
            guard let swiftResult = result else {
                XCTFail("Result should not be nil.")
                return
            }
            guard let result = try? swiftResult.get() else {
                XCTFail("Result should have data.")
                return
            }
    ...
    func testAppleWithAllFields() {
        ...
        Service.default.fetch(query: "apple.com", fields: Service.Result.Field.all) {
            result = $0
            responseArrived.fulfill()
        }
    ...
    func testAppleWithAllFieldsInSpanish() {
        ...
        Service.default.fetch(query: "apple.com", fields: Service.Result.Field.all, language: "es") {
            result = $0
            responseArrived.fulfill()
        }
    ...
    func testFailed() {
        ...
        Service.default.fetch(query: "failed.lookup") {
            result = $0
            responseArrived.fulfill()
        }
